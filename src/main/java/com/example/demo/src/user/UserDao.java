...
public class UserDao {
    ...
    public int checkDeviceNum(String deviceNum) {
        String checkDeviceNumQuery = "SELECT userIdx FROM User WHERE deviceNum = ?;"; // User Table에 해당 deviceNum 값을 갖는 유저 정보가 존재하는가?
        String checkDeviceNumParams = deviceNum;
        try {
            return this.jdbcTemplate.queryForObject(checkDeviceNumQuery,
                    int.class,
                    checkDeviceNumParams);
        } catch (EmptyResultDataAccessException e) {
            // 지정된 deviceNum에 해당하는 레코드가 없는 경우 처리
            return 0;
        }
    }
